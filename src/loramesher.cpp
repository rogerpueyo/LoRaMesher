...
void LoraMesher::initializeNetwork() {
    int res = xTaskCreate([](void* o) { static_cast<LoraMesher*>(o)->sendHelloPacket(); },
                          "Hello routine",
                          4096,
                          this,
                          0,
...
void LoraMesher::initializeLoRa() {
    ...
    res = xTaskCreate([](void* o) { static_cast<LoraMesher*>(o)->receivingRoutine(); },
                      "Receiving routine",
                      4096,
                      this,
                      0,
...
void LoraMesher::sendHelloPacket() {
    for (;;) {
        ...
        radio->clearDio0Action(); // For some reason, while transmitting packets, the interrupt pin
                                  // is set with a ghost packet
        packet tx;
        ...
        // TODO: Change this to startTransmit as a mitigation to the wdt error so we can raise the
        // priority of the task. We'll have to look on how to start to listen for the radio again
        ...
        // TODO: Change this to vTaskDelayUntil to prevent sending too many packets as this is not
        // considering normal data packets sent for legal purposes
...
void LoraMesher::sendDataPacket() {
    ...
    radio->clearDio0Action(); // For some reason, while transmitting packets, the interrupt pin is
                              // set with a ghost packet
...
void IRAM_ATTR LoraMesher::onReceive() {
    ...
    xTaskNotifyFromISR(
        ReceivePacket_TaskHandle, 0, eSetValueWithoutOverwrite, &xHigherPriorityTaskWoken);
    ...
    for (;;) {
        ...
        if (TWres == pdPASS) {
            ...
            if (packetSize == 0)
                ...
                Log.notice(F("Receiving LoRa packet %d: Size: %d RSSI: %d SNR: %d" CR),
                           receivedPackets,
                           packetSize,
                           rssi,
                           snr);
                ...
                if (rx.dst == broadcastAddress) {
                    if (rx.type == HELLO_P) {
                        ...
                        case HOPCOUNT:
                            ...
                            if (!isNodeInRoutingTable(rx.src)) {
                                Log.verbose(F("Adding new neighbour %X to the routing table" CR),
                                            rx.src);
                            }
                    ...
                    Log.verbose(
                        F("Packet from %X for %X (not for me). IGNORING" CR), rx.src, rx.dst);
                }
...
void LoraMesher::processRoute(byte sender, int helloseqnum, int rssi, int snr, byte addr,
                              int mtrc) {
            ...
            if (!knownAddr) {
                for (int i = 0; i < RTMAXSIZE; i++) {
                    if (routingTable[i].address == 0) {
                        ...
                        routingTable[i].via = sender;
                        Log.verbose(
                            F("New route added: %X via %X metric %d" CR), addr, sender, mtrc);
                        break;
                    }
                }
            }
